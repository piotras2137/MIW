...
def svd(matrix):
    ...
    if AAT_S > ATA_S:
        ...
        wartosci_singularne = wartosci_singularne_z_wartosci_wlasnych(
            wartosci_wlasne)
        U = sortuj_wiersze(np.linalg.eigh(AAT)[1])
        U = np.array([unifikacja(x) for x in U])
        V = np.array([(np.dot(matrix.T, U.T[x]) * (1/wartosci_singularne[x]))
                     for x in range(ATA_S)])
        ...
        V = np.array([unifikacja(x) for x in V])
        U = np.array([(np.dot(matrix, V.T[x]) * (1/wartosci_singularne[x]))
                     for x in range(AAT_S)])
        U = np.array([unifikacja(x) for x in U])
        U = sortuj_wiersze(U)
